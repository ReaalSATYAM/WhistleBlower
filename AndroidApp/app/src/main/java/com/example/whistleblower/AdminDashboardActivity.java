...
import com.example.whistleblower.LocalReportStore;
...
public class AdminDashboardActivity extends AppCompatActivity {
    ...
    private void loadDummyReports() {
        ...
        // Load locally submitted reports (from MainActivity)
        if (!LocalReportStore.reports.isEmpty()) {
            for (LocalReportStore.Report report : LocalReportStore.reports) {
                addReportCard(report.title, report.desc, report.status);
            }
        }
        //  Fallback dummy data
        else {
            addReportCard(
                    "Financial Misconduct in Dept A",
                    "Suspicious transactions observed in the ledger...",
                    "pending"
            );

            addReportCard(
                    "Bribery Evidence",
                    "Video proof of officer accepting cash.",
                    "accepted"
            );
    ...
    private void addReportCard(String title, String desc, String status) {
        ...
        // Determine the index of this report in LocalReportStore
        int reportIndex = LocalReportStore.reports.size() - 1; // last added report

        btnReview.setOnClickListener(v -> {
            Intent intent = new Intent(AdminDashboardActivity.this, EvidenceReviewActivity.class);
            intent.putExtra("report_index", reportIndex); // pass index to review activity
    ...
    protected void onResume() {
        super.onResume();
        layoutReportList.removeAllViews(); // clear old cards
        loadDummyReports(); // reload all reports with updated status
    }
